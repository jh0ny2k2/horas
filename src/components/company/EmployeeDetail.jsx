            ...
            {filteredHistory.length === 0 ? (
              ...
              <div className="space-y-2">
                {filteredHistory.map(shift => (
                  <div key={shift.id} className="flex items-center justify-between py-2.5 border-b border-slate-100 dark:border-slate-800 last:border-0">
                    <div className="flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-full ${shift.approved ? 'bg-emerald-500' : 'bg-amber-500'}`}></div>
                      <div>
                        <p className="text-sm font-medium text-slate-700 dark:text-slate-200">{formatDate(shift.work_date)}</p>
                        <p className="text-xs text-slate-400 dark:text-slate-500">{shift.start_time.slice(0, 5)} - {shift.end_time.slice(0, 5)}{shift.break_minutes > 0 && ` · ${shift.break_minutes}min`}{!shift.approved && ' · Pendiente'}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-bold text-slate-700 dark:text-slate-200 tabular-nums">{formatHours(shift.total_hours)}h</span>
                      {shift.approved ? (
                        <button onClick={() => handleReject(shift.id)} className="p-1 rounded-lg text-slate-400 dark:text-slate-500 hover:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-colors" title="Desaprobar">
                          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                        </button>
                      ) : (
                        <button onClick={() => handleApprove(shift.id)} className="p-1 rounded-lg text-slate-400 dark:text-slate-500 hover:text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 transition-colors" title="Aprobar">
