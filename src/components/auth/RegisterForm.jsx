    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-ivory dark:bg-slate-950 px-6 py-12">
        <div className="w-full max-w-sm space-y-8 text-center">
          <div className="mx-auto w-16 h-16 rounded-full bg-emerald-100 dark:bg-emerald-500/15 flex items-center justify-center">
            <svg className="w-8 h-8 text-emerald-600 dark:text-emerald-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
            </svg>
          </div>
          <div className="space-y-2">
            <h2 className="text-3xl font-medium tracking-tight text-slate-800 dark:text-white">
              Cuenta creada
            </h2>
            <p className="text-[15px] text-slate-400 dark:text-slate-500">
              Hemos enviado un enlace de confirmación a <strong className="text-slate-600 dark:text-slate-300">{email}</strong>.
            </p>
          </div>
          <button
            onClick={() => navigate(joinToken ? `/join/${joinToken}` : '/auth/login')}
            className="w-full h-12 rounded-full bg-brand-600 dark:bg-brand-500 text-white font-medium text-base flex items-center justify-center gap-2 hover:bg-brand-700 dark:hover:bg-brand-600 active:scale-[0.99] transition-all duration-150"
          >
      ...
      <div className="w-full max-w-sm space-y-10">
        ...
        <div className="space-y-5">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-xs font-medium text-slate-400 dark:text-slate-500">o regístrate con</span>
            <span className="flex-1 h-px bg-slate-200 dark:bg-slate-800" aria-hidden="true" />
          </div>
        </div>
        ...
        <p className="text-center text-sm text-slate-400 dark:text-slate-500">
          ¿Ya tienes cuenta?{' '}
          <Link to={joinToken ? `/auth/login?join=${joinToken}` : '/auth/login'} className="font-semibold text-slate-800 dark:text-white hover:underline underline-offset-4">
            Iniciar sesión
          </Link>
        </p>
      </div>
